...
class HeadCountAnalystTest < Minitest::Test
  ...
  def test_if_kindergarten_participation_avg_compares_to_state_average
    assert_equal 0.766, @headcount_analyst.calculate_data_variation('ACADEMY 20', 'COLORADO', :kindergarten_participation)
  end

  def test_if_kindergarten_participation_compares_to_another_district
    assert_equal 0.573, @headcount_analyst.calculate_data_variation('ACADEMY 20', 'ADAMS COUNTY 14', :kindergarten_participation)
  end

  def test_if_kindergarten_participation_rate_trends_against_state_average
  ...
  def test_if_hs_graduation_avg_compares_to_state_average
    assert_equal 1.194, @headcount_analyst.calculate_data_variation("ACADEMY 20", 'COLORADO', :high_school_graduation_rates)
  end

  def test_does_kindergarten_participation_affect_hs_graduation
    assert_equal 0.642, @headcount_analyst.calculate_kinder_to_hs_retention_variation('ACADEMY 20')
  end

  def test_does_kindergarten_participation_correlate
  ...
  def test_do_statewide_percentages_correlate_with_grad_rates
    assert_equal true, @headcount_analyst.does_statewide_kindergarten_participation_correlate_with_district_hs_graduation('COLORADO')
  end

  def test_does_kindergarten_participation_correlate_with_a_subset_of_districts
    districts = ['ACADEMY 20','ADAMS COUNTY 14', 'ADAMS-ARAPAHOE 28J', 'AGUILAR REORGANIZED 6']
    
    assert_equal true, @headcount_analyst.does_kindergarten_participation_correlate_with_high_school_graduation_across_subset_of_districts(districts, "COLORADO")
  end
end
